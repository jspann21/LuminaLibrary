        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="min-w-0 flex-1 pt-1">
              {isEditing ? (
                <div className="space-y-4">
                  <div><label className="mb-1 block text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Title</label><input className="w-full rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm dark:border-slate-700 dark:bg-slate-800/50" value={form.title} onChange={(event) => updateForm((current) => ({ ...current, title: event.target.value }))} /></div>
                  <div><label className="mb-1 block text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Subtitle</label><input className="w-full rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm dark:border-slate-700 dark:bg-slate-800/50" placeholder="Optional" value={form.subtitle} onChange={(event) => updateForm((current) => ({ ...current, subtitle: event.target.value }))} /></div>
                  <div><label className="mb-1 block text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Author(s)</label><input className="w-full rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm dark:border-slate-700 dark:bg-slate-800/50" value={form.authors} onChange={(event) => updateForm((current) => ({ ...current, authors: event.target.value }))} /></div>
                </div>
              ) : (
                <>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">Publisher</span>{isEditing ? <input className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.publisher} onChange={(event) => updateForm((current) => ({ ...current, publisher: event.target.value }))} /> : <span className="block truncate font-mono text-sm text-slate-700 dark:text-slate-300">{book.publisher || '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">Published</span>{isEditing ? <input type="date" className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.publishDate} onChange={(event) => updateForm((current) => ({ ...current, publishDate: event.target.value }))} /> : <span className="font-mono text-sm text-slate-700 dark:text-slate-300">{book.publishDate ? formatDate(book.publishDate) : '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">ISBN-13</span>{isEditing ? <input className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.isbn13} onChange={(event) => updateForm((current) => ({ ...current, isbn13: event.target.value }))} /> : <span className="font-mono text-sm text-slate-700 dark:text-slate-300">{book.isbn13 || '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">ISBN-10</span>{isEditing ? <input className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.isbn10} onChange={(event) => updateForm((current) => ({ ...current, isbn10: event.target.value }))} /> : <span className="font-mono text-sm text-slate-700 dark:text-slate-300">{book.isbn10 || '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">Pages</span>{isEditing ? <input type="number" className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.pageCount} onChange={(event) => updateForm((current) => ({ ...current, pageCount: event.target.value }))} /> : <span className="font-mono text-sm text-slate-700 dark:text-slate-300">{book.pageCount ?? '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">Series</span>{isEditing ? <div className="flex gap-2"><input className="min-w-0 flex-1 border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.series} onChange={(event) => updateForm((current) => ({ ...current, series: event.target.value }))} /><input type="number" className="w-12 border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.seriesIndex} onChange={(event) => updateForm((current) => ({ ...current, seriesIndex: event.target.value }))} /></div> : <span className="block truncate font-mono text-sm text-slate-700 dark:text-slate-300">{book.series ? `${book.series}${book.seriesIndex ? ` #${book.seriesIndex}` : ''}` : '—'}</span>}</div>
            </div>

            <div>
              <h3 className="mb-2 font-semibold text-slate-900 dark:text-white">Description</h3>
              {isEditing ? <textarea className="h-40 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-700 dark:bg-slate-800/50" value={form.description} onChange={(event) => updateForm((current) => ({ ...current, description: event.target.value }))} /> : <p className="whitespace-pre-wrap text-sm leading-relaxed text-slate-600 dark:text-slate-400">{book.description || 'No description available.'}</p>}
            </div>
