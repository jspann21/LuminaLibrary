        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="min-w-0 flex-1 pt-1">
              {isEditing ? (
                <div className="space-y-4">
                  <div><label htmlFor={`book-detail-${book.id}-title`} className="mb-1 block text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Title</label><input id={`book-detail-${book.id}-title`} className="w-full rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm dark:border-slate-700 dark:bg-slate-800/50" value={form.title} onChange={(event) => updateForm((current) => ({ ...current, title: event.target.value }))} /></div>
                  <div><label htmlFor={`book-detail-${book.id}-subtitle`} className="mb-1 block text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Subtitle</label><input id={`book-detail-${book.id}-subtitle`} className="w-full rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm dark:border-slate-700 dark:bg-slate-800/50" placeholder="Optional" value={form.subtitle} onChange={(event) => updateForm((current) => ({ ...current, subtitle: event.target.value }))} /></div>
                  <div><label htmlFor={`book-detail-${book.id}-authors`} className="mb-1 block text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Author(s)</label><input id={`book-detail-${book.id}-authors`} className="w-full rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm dark:border-slate-700 dark:bg-slate-800/50" value={form.authors} onChange={(event) => updateForm((current) => ({ ...current, authors: event.target.value }))} /></div>
                </div>
              ) : (
                <>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <span className="flex" title={primaryFile ? 'Open file' : 'Requires a linked local file'}>
              ...
              <span className="flex" title={isHiding ? 'Hiding book' : 'Hide from library'}>
              ...
              <span className="flex" title={isDeleting ? 'Deleting book' : 'Delete book'}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><label htmlFor={`book-detail-${book.id}-publisher`} className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">Publisher</label>{isEditing ? <input id={`book-detail-${book.id}-publisher`} className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.publisher} onChange={(event) => updateForm((current) => ({ ...current, publisher: event.target.value }))} /> : <span className="block truncate font-mono text-sm text-slate-700 dark:text-slate-300">{book.publisher || '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><label htmlFor={`book-detail-${book.id}-publish-date`} className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">Published</label>{isEditing ? <input id={`book-detail-${book.id}-publish-date`} type="date" className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.publishDate} onChange={(event) => updateForm((current) => ({ ...current, publishDate: event.target.value }))} /> : <span className="font-mono text-sm text-slate-700 dark:text-slate-300">{book.publishDate ? formatDate(book.publishDate) : '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><label htmlFor={`book-detail-${book.id}-isbn13`} className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">ISBN-13</label>{isEditing ? <input id={`book-detail-${book.id}-isbn13`} className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.isbn13} onChange={(event) => updateForm((current) => ({ ...current, isbn13: event.target.value }))} /> : <span className="font-mono text-sm text-slate-700 dark:text-slate-300">{book.isbn13 || '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><label htmlFor={`book-detail-${book.id}-isbn10`} className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">ISBN-10</label>{isEditing ? <input id={`book-detail-${book.id}-isbn10`} className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.isbn10} onChange={(event) => updateForm((current) => ({ ...current, isbn10: event.target.value }))} /> : <span className="font-mono text-sm text-slate-700 dark:text-slate-300">{book.isbn10 || '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><label htmlFor={`book-detail-${book.id}-page-count`} className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">Pages</label>{isEditing ? <input id={`book-detail-${book.id}-page-count`} type="number" className="w-full border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.pageCount} onChange={(event) => updateForm((current) => ({ ...current, pageCount: event.target.value }))} /> : <span className="font-mono text-sm text-slate-700 dark:text-slate-300">{book.pageCount ?? '—'}</span>}</div>

              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/50"><label htmlFor={`book-detail-${book.id}-series`} className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-400">Series</label>{isEditing ? <div className="flex gap-2"><input id={`book-detail-${book.id}-series`} className="min-w-0 flex-1 border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.series} aria-label="Series title" onChange={(event) => updateForm((current) => ({ ...current, series: event.target.value }))} /><input id={`book-detail-${book.id}-series-index`} type="number" className="w-12 border-b border-slate-300 bg-transparent text-sm dark:border-slate-600" value={form.seriesIndex} aria-label="Series number" onChange={(event) => updateForm((current) => ({ ...current, seriesIndex: event.target.value }))} /></div> : <span className="block truncate font-mono text-sm text-slate-700 dark:text-slate-300">{book.series ? `${book.series}${book.seriesIndex ? ` #${book.seriesIndex}` : ''}` : '—'}</span>}</div>
            </div>

            <div>
              <label htmlFor={`book-detail-${book.id}-description`} className="mb-2 block font-semibold text-slate-900 dark:text-white">Description</label>
              {isEditing ? <textarea id={`book-detail-${book.id}-description`} className="h-40 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-700 dark:bg-slate-800/50" value={form.description} onChange={(event) => updateForm((current) => ({ ...current, description: event.target.value }))} /> : <p className="whitespace-pre-wrap text-sm leading-relaxed text-slate-600 dark:text-slate-400">{book.description || 'No description available.'}</p>}
            </div>
